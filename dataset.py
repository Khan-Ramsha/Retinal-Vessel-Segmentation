...
class CustomDataset(Dataset):
    def __init__(self, image_paths, mask_paths, transform=None, use_preprocessing=True):
        self.image_paths = image_paths
        self.mask_paths = mask_paths
        self.img_lists = sorted(os.listdir(image_paths))
        self.mask_lists = sorted(os.listdir(mask_paths))
    ...
    def __getitem__(self, idx):
        ...
        mask = Image.open(os.path.join(self.mask_paths, self.mask_lists[idx])).convert('L')
        ...
        if self.use_preprocessing:
            ...
            mask = cv2.bitwise_and(mask, mask, mask=fov_mask)
        ...
        mask = (mask > 0.5).float().unsqueeze(0) if len(mask.shape) == 2 else mask
